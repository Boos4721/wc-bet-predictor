...
function setMatches(list: Match[]): void {
  ...
  prediction = null;
  renderMatches(true); renderPredEmpty();
}
...
function addLegFromMatch(which: "home" | "draw" | "away"): void {
  const sel = el<HTMLSelectElement>("calcMatch");
  const m = matches.filter((x) => x.id === sel.value)[0];
  if (!m) return;
  const odds = which === "home" ? m.odds.home : which === "draw" ? m.odds.draw : m.odds.away;
  if (!(odds > 1)) return; // 无该结果盘口(如 2-way 无平局)
  const zh = which === "home" ? "主胜" : which === "draw" ? "平局" : "客胜";
  addCalcLeg(`${m.id} ${m.home} vs ${m.away} ${zh}`, odds);
}
